...
if(GetVars('act','GET')=='CateArticleEdt')$action = 'CateArticleEdt';
if (!$zbp->CheckRights($action)) {
  $zbp->ShowError(6,__FILE__,__LINE__);die();
//   echo "myaction".$action;
  die();
}
...
<input id="postType" name="postType" type="text"  style="display:none;" value="<?php echo $action?>">
...
?>
    ...
    <div id="divEditTitle" class="editmod2">
      ...
      <input type="hidden" name="cate" value="<?php echo $article->CateID; ?>">
      <!-- title( -->
      ...
      if($article->ID == 0)
      {
		...
		$log_ID = $article->ID;
        $sqlAbstract = "SELECT * FROM ubc_postaddition WHERE log_ID = '$log_ID'";
        $reAbstract = $zbp->db->Query($sqlAbstract);
        $abstract = $reAbstract[0]['log_abstract'];
	  }
      ?>
      <div id="abstract" class="editmod" >
           <label for="edtAbstract" class="editinputname">文章摘要</label>
           <div style="width:100%;" ><textarea style="width:100%; resize: none;height:50px;"  type="text" name="Abstract" id="edtAbstract"><?php echo $abstract;?></textarea></div>
      </div>
      <!-- abstract) -->
      <!-- record( -->
      <div id="record" class="editmod">
           <label for="edtAbstract" class="editinputname">语音记录</label>
           <input type="file" name="Record">
           (只允许上传 mp3, waw, ogg格式录音)
      </div>

      <!-- record) -->
      
      <!-- titlePic( -->
      <div id="pic" class="editmot">
           <label for="edtAbstract" class="editinputname">标题图片</label>
           <input type="file" name="Picture">
           (只允许上传 bmp, jpg, jpeg, gif, png格式图片)
      </div>
      <!-- titlePic) -->
...
?>
      ...
      <!-- alias( -->
      <div style="display:none;" id="alias" class="editmod2"><label for="edtAlias" class="editinputname" ><?php echo $lang['msg']['alias']?></label>
    ...
    <div id="divEditPost">
      <div id="divBox">
        <div id="divFloat">
          ...
          <!-- cate --><?php  if(!$ispage && $action!='ArticleEdt'){ ?>
          <div id='cate' class="editmod"> <label for="cmbCateID" class="editinputname" ><?php echo $lang['msg']['category']?></label>
            <select style="width:150px;" class="edit" size="1" name="CateID" id="cmbCateID">
<?php  echo CreateOptoinsOfCategorys($article->CateID);?>
            </select>
          </div>
          <!-- cate --><?php }elseif($action!='CateArticleEdt'){

          	?>

          <!--  tags -->
          <div id='tag' class="editmod"> <label for="edtTag" class="editinputname" ><?php echo $lang['msg']['tags']?></label>
            <select style="width:150px;" class="edit" size="1" name="Tag" id="cmbTagID">
<?php echo CreateOptoinsOfTags($article->Tag);?>
            </select>
          </div>
          <?php }?>
          
          <!-- level -->
          <div style="display: none" id='level' class="editmod"> <label for="cmbPostStatus" class="editinputname" ><?php echo $lang['msg']['status']?></label>
...
<?php echo CreateOptoinsOfPostStatus($article->Status);?>
          ...
          <div style="display: none" id='template' class="editmod"> <label for="cmbTemplate" class="editinputname" ><?php echo $lang['msg']['template']?></label>
...
<?php echo CreateOptoinsOfTemplate($article->Template);?>
          ...
          <div style="display: none" id='AddNavbar' class="editmod">
...
elseif($action=='CateArticleEdt'){
	if($article->ID==0){
	echo '<script type="text/javascript">ActiveLeftMenu("aCateArticleEdt");</script>';
	}
	else{
...
if($action == "ArticleEdt")
{
	...
	echo 'document.getElementById("pic").style.display = "block";';
	echo 'document.getElementById("record").style.display = "none";';
	echo '</script>';
}

if($action == "CateArticleEdt")
{
	echo '<script type="text/javascript">';
	echo 'document.getElementById("pic").style.display = "none";';
	echo 'document.getElementById("record").style.display = "none";';
	echo '</script>';
}
?>
...
function checkArticleInfo(){
  ...
  if(type == "CateArticleEdt")
  {
// 	  alert(type == "CateArticleEdt");
	 document.getElementById("edit").action="<?php echo '../cmd.php?act=CateArticlePst'?>";
  }else  
...
$(document).ready(function(){ 
	var p = $("#cmbTagID option:selected").val();
	if(p == 1)
	{
...
$("#edit").submit(function(){
	if($("#record").css("display") != "none"){
		var filepath=$("input[name='Record']").val();
		var extStart=filepath.lastIndexOf(".");
	    var ext=filepath.substring(extStart,filepath.length).toUpperCase();
	    if(ext!=undefined && ext!=null && ext!="" && ext!=".MP3"&&ext!=".WAW"&&ext!=".OGG"){
	        alert("录音限于mp3,waw,ogg格式");
			return false;
	    }
	}
	if($("#pic").css("display") != "none"){
		var filepath=$("input[name='Picture']").val();
		var extStart=filepath.lastIndexOf(".");
	    var ext=filepath.substring(extStart,filepath.length).toUpperCase();
	    if(ext!=undefined && ext!=null && ext!="" && ext!=".BMP"&&ext!=".PNG"&&ext!=".GIF"&&ext!=".JPG"&&ext!=".JPEG"){
