...
switch ($action) {
	...
	case 'CateArticleDel':
		...
		$zbp->BuildModule();
		$zbp->SetHint('good');
		Redirect('cmd.php?act=CateArticleMng');
		break;
	case 'CateArticlePst'://提交文章
		PostCateArticle();
		$zbp->BuildModule();
		$zbp->SetHint('good');
		Redirect('cmd.php?act=CateArticleMng');
	...
	case 'CategoryEdt':
		...
		break;
	case 'TopCategoryEdt':
		Redirect('admin/topcategory_edit.php?' . GetVars('QUERY_STRING','SERVER'));
		break;
	case 'CategoryPst':
		PostCategory();
		$zbp->BuildModule();
	...
	case 'TopCategoryPst':
		...
		$zbp->BuildModule();
		$zbp->SetHint('good');
		Redirect('cmd.php?act=TopCategoryMng');
		break;
	case 'CategoryDel':
		if(!$zbp->ValidToken(GetVars('token','GET'))){$zbp->ShowError(5,__FILE__,__LINE__);die();}
		DelCategory();
