...
function Zbp_Admin_Addtopcatesubmenu(){
	echo '<a href="../cmd.php?act=TopCategoryEdt"><span class="m-left">' . $GLOBALS['lang']['msg']['new_top_category'] . '</span></a>';
}
...
Add_Filter_Plugin('Filter_Plugin_Admin_DirPageMng_SubMenu','Zbp_Admin_Adddirpagesubmenu');


Add_Filter_Plugin('Filter_Plugin_Admin_TopCategoryMng_SubMenu','Zbp_Admin_Addtopcatesubmenu');
...
function ResponseAdmin_LeftMenu(){
	...
	//新建目录文章
	$leftmenus['nav_newcatea'] = MakeLeftMenu("CateArticleEdt",$zbp->lang['msg']['nav_newcatearticle'],$zbp->host . "zb_system/cmd.php?act=CateArticleEdt","nav_newcatea","aCateArticleEdt","");
// 	echo $leftmenus['nav_newcatea'];
	...
	$leftmenus['nav_catearticle']=MakeLeftMenu("CateArticleMng",$zbp->lang['msg']['catearticle_manage'],$zbp->host . "zb_system/cmd.php?act=CateArticleMng","nav_catearticle","aCateArticleMng","");
	...
	$leftmenus['nav_top_category'] = MakeLeftMenu("TopCategoryMng",$zbp->lang['msg']['top_category_manage'],$zbp->host . "zb_system/cmd.php?act=TopCategoryMng","nav_top_category","aTopCategoryMng","");
...
function CreateOptoinsOfCategorys($default){
	...
	$s=null;
	foreach ($zbp->newCategorys as $id => $cate) {
	  $s.='<option ' . (($default==0 || $default==$cate['cate_ID'])?'selected="selected"':'') . ' value="'. $cate['cate_ID'] .'">' . $cate['cate_Name'] . '</option>';
...
function createOptoinsOfTags($default){
	global $zbp;
	$s=null;
	foreach ($zbp->newTags as $id => $tag) {
		$s.='<option ' . (($default==null || $default== $tag['tag_ID'])?'selected="selected"':'') . ' value="'. $tag['tag_ID'] .'">' . $tag['tag_Name'] . '</option>';
...
foreach ($array as $article) {
	...
	echo '<td class="td10">' . $article->TagsName . '</td>';		//=============================
...
/**
 * 后台目录文章管理 --- by xiazdong
 */
function Admin_CateArticleMng(){

	global $zbp;

	echo '<div class="divHeader">' . $zbp->lang['msg']['catearticle_manage'] . '</div>';
	echo '<div class="SubMenu">';
	foreach ($GLOBALS['Filter_Plugin_Admin_CateArticleMng_SubMenu'] as $fpname => &$fpsignal) {
		$fpname();
	}
	echo '</div>';
	echo '<div id="divMain2">';
	...
	foreach ($array as $article) {
		...
		echo '<td class="td5">' . ($article->IsTop?$zbp->lang['msg']['top'].'|':'').$article->StatusName . '</td>';
		echo '<td class="td10 tdCenter">';
		echo '<a href="../cmd.php?act=CateArticleEdt&amp;id='. $article->ID .'"><img src="../image/admin/page_edit.png" alt="'.$zbp->lang['msg']['edit'] .'" title="'.$zbp->lang['msg']['edit'] .'" width="16" /></a>';
		echo '&nbsp;&nbsp;&nbsp;&nbsp;';
		echo '<a onclick="return window.confirm(\''.$zbp->lang['msg']['confirm_operating'] .'\');" href="../cmd.php?act=CateArticleDel&amp;id='. $article->ID .'&amp;token='. $zbp->GetToken() .'"><img src="../image/admin/delete.png" alt="'.$zbp->lang['msg']['del'] .'" title="'.$zbp->lang['msg']['del'] .'" width="16" /></a>';
		echo '</td>';
	...
	echo '</p></div>';
	echo '<script type="text/javascript">ActiveLeftMenu("aCateArticleMng");</script>';
	echo '<script type="text/javascript">AddHeaderIcon("'. $zbp->host . 'zb_system/image/common/article_32.png' . '");</script>';
...
function Admin_CategoryMng(){
	...
	<th>' . $zbp->lang['msg']['id'] . '</th>
	<th style="display:none">' . $zbp->lang['msg']['order'] . '</th>
	<th>' . $zbp->lang['msg']['name'] . '</th>
	<th>' . $zbp->lang['msg']['parent'] . '</th>
 	<th style="display:none">' . $zbp->lang['msg']['alias'] . '</th> 
	<th>' . $zbp->lang['msg']['post_count'] . '</th>
	<th></th>
	</tr>';	
	

foreach ($zbp->newCategorys as $category) {
	echo '<tr>';
	echo '<td class="td5">' . $category['cate_ID'] . '</td>';
	echo '<td class="td5" style="display:none">' . $category['cate_Order']. '</td>';
	echo '<td class="td25"><a href="'.$category->Url .'" target="_blank"><img src="../image/admin/link.png" alt="" title="" width="16" /></a> ' . /* $category->Symbol . */ $category['cate_Name'] . '</td>';
	echo '<td class="td10">' .$category['topcate_Name'] . '</td>';
	echo '<td class="td20" style="display:none">' . $category['cate_Alias'] . '</td>';
	echo '<td class="td10">' . $category['cate_Count']. '</td>';
	echo '<td class="td10 tdCenter">';
	echo '<a href="../cmd.php?act=CategoryEdt&amp;id='. $category['cate_ID'] .'"><img src="../image/admin/folder_edit.png" alt="'.$zbp->lang['msg']['edit'] .'" title="'.$zbp->lang['msg']['edit'] .'" width="16" /></a>';
	echo '&nbsp;&nbsp;&nbsp;&nbsp;';
	echo '<a onclick="return window.confirm(\''.$zbp->lang['msg']['confirm_operating'] .'\');" href="../cmd.php?act=CategoryDel&amp;id='. $category['cate_ID'] .'&amp;token='. $zbp->GetToken() .'"><img src="../image/admin/delete.png" alt="'.$zbp->lang['msg']['del'] .'" title="'.$zbp->lang['msg']['del'] .'" width="16" /></a>';
	echo '</td>';
...
function Admin_TopCategoryMng(){

	global $zbp;

	echo '<div class="divHeader">' . $zbp->lang['msg']['top_category_manage'] . '</div>';
	echo '<div class="SubMenu">';
 	foreach ($GLOBALS['Filter_Plugin_Admin_TopCategoryMng_SubMenu'] as $fpname => &$fpsignal) {
		$fpname();
	} 
	echo '</div>';
	echo '<div id="divMain2">';
	...
	foreach ($zbp->newTopCategorys as $k=>$category) {
		echo '<tr>';
		echo '<td class="td5">' . $category['topcate_ID'] . '</td>';
		echo '<td class="td5">' . $category['topcate_Order']. '</td>';
		echo '<td class="td5">' . $category['topcate_Name']. '</td>';
		echo '<td class="td10 tdCenter">';
		echo '<a href="../cmd.php?act=TopCategoryEdt&amp;id='. $category['topcate_ID'] .'"><img src="../image/admin/folder_edit.png" alt="'.$zbp->lang['msg']['edit'] .'" title="'.$zbp->lang['msg']['edit'] .'" width="16" /></a>';
		echo '&nbsp;&nbsp;&nbsp;&nbsp;';
		echo '<a onclick="return window.confirm(\''.$zbp->lang['msg']['confirm_operating'] .'\');" href="../cmd.php?act=TopCategoryDel&amp;id='. $category['topcate_ID'] .'&amp;token='. $zbp->GetToken() .'"><img src="../image/admin/delete.png" alt="'.$zbp->lang['msg']['del'] .'" title="'.$zbp->lang['msg']['del'] .'" width="16" /></a>';
		echo '</td>';
	...
	echo '</table>';
	echo '</div>';
	echo '<script type="text/javascript">ActiveLeftMenu("aTopCategoryMng");</script>';
	echo '<script type="text/javascript">AddHeaderIcon("'. $zbp->host . 'zb_system/image/common/category_32.png' . '");</script>';
...
function Admin_TagMng(){
	...
	<th>' . $zbp->lang['msg']['name'] . '</th>
	<th style="display:none">' . $zbp->lang['msg']['alias'] . '</th>
	<th style="display:none">' . $zbp->lang['msg']['post_count'] . '</th>
	<th></th>
	</tr>';
...
foreach ($array as $tag) {
	...
	echo '<td style="display:none" class="td20">' . $tag->Alias . '</td>';
	echo '<td style="display:none" class="td10">' . $tag->Count . '</td>';
	echo '<td class="td10 tdCenter">';
...
<?php
	...
	echo '<tr style="display:none"><td class="td25"><p><b>'.$zbp->lang['msg']['blog_host'].'</b><br/><span class="note">&nbsp;'.$zbp->lang['msg']['blog_host_add'].'</span></p></td><td><p><input id="ZC_BLOG_HOST" name="ZC_BLOG_HOST" style="width:600px;" type="text" value="'.$zbp->option['ZC_BLOG_HOST'].'" '.($zbp->option['ZC_PERMANENT_DOMAIN_ENABLE']?'':'readonly="readonly"').' />';
	...
	echo '<tr style="display:none"><td><p><b>'.$zbp->lang['msg']['blog_subname'].'</b></p></td><td><p><input id="ZC_BLOG_SUBNAME" name="ZC_BLOG_SUBNAME" style="width:600px;"  type="text" value="'.$zbp->option['ZC_BLOG_SUBNAME'].'" /></p></td></tr>';
	echo '<tr><td><p><b>'.$zbp->lang['msg']['copyright'].'</b><br/><span style="display:none" class="note">&nbsp;'.$zbp->lang['msg']['copyright_add'].'</span></p></td><td><p><textarea cols="3" rows="6" id="ZC_BLOG_COPYRIGHT" name="ZC_BLOG_COPYRIGHT" style="width:600px;">'.htmlspecialchars($zbp->option['ZC_BLOG_COPYRIGHT']).'</textarea></p></td></tr>';
    
	//#增加
// 	echo '<tr><td rows="10"><p><b>'.'友情连接1'.'</b></p>'.'</td>';
// 	echo '<td>名字：</lable><input cols="1" rows="1" id="ADD_NAME_1" name="ADD_NAME_1" style="width:100px;"><label>地址：</label><input cols="1" rows="1" id="FRIEND_ADDRESS_1" name="FRIEND_ADDRESS_1" style="width:100px;">'.'</textarea></td>';
// 	echo '</tr>';
// 	echo '<tr><td rows="10"><p><b>'.'友情连接2'.'</b></p>'.'</td>';
// 	echo '<td>名字：</lable><input cols="1" rows="1" id="ADD_NAME_2" name="ADD_NAME_2" style="width:100px;"><label>地址：</label><input cols="1" rows="1" id="FRIEND_ADDRESS_2" name="FRIEND_ADDRESS_2" style="width:100px;">'.'</textarea></td>';
// 	echo '</tr>';
// 	echo '<tr><td rows="10"><p><b>'.'友情连接3'.'</b></p>'.'</td>';
// 	echo '<td>名字：</lable><input cols="1" rows="1" id="ADD_NAME_3" name="ADD_NAME_3" style="width:100px;"><label>地址：</label><input cols="1" rows="1" id="FRIEND_ADDRESS_3" name="FRIEND_ADDRESS_3" style="width:100px;">'.'</textarea></td>';
// 	echo '</tr>';
// 	echo '<tr><td rows="10"><p><b>'.'友情连接4'.'</b></p>'.'</td>';
// 	echo '<td>名字：</lable><input cols="1" rows="1" id="ADD_NAME_4" name="ADD_NAME_4" style="width:100px;"><label>地址：</label><input cols="1" rows="1" id="FRIEND_ADDRESS_3" name="FRIEND_ADDRESS_4" style="width:100px;">'.'</textarea></td>';
...
// 	echo '</tr>';
	...
	echo '<tr style="display:none"><td><p><b>'.$zbp->lang['msg']['allow_upload_type'].'</b></p></td><td><p><input id="ZC_UPLOAD_FILETYPE" name="ZC_UPLOAD_FILETYPE" style="width:600px;" type="text" value="'.$zbp->option['ZC_UPLOAD_FILETYPE'].'" /></p></td></tr>';
	echo '<tr style="display:none"><td><p><b>'.$zbp->lang['msg']['allow_upload_size'].'</b></p></td><td><p><input id="ZC_UPLOAD_FILESIZE" name="ZC_UPLOAD_FILESIZE" style="width:600px;" type="text" value="'.$zbp->option['ZC_UPLOAD_FILESIZE'].'" /></p></td></tr>';
	echo '<tr style="display:none"><td><p><b>'.$zbp->lang['msg']['debug_mode'].'</b></p></td><td><p><input id="ZC_DEBUG_MODE" name="ZC_DEBUG_MODE" type="text" value="'.$zbp->option['ZC_DEBUG_MODE'].'" class="checkbox"/></p></td></tr>';
	echo '<tr style="display:none"><td><p><b>'.$zbp->lang['msg']['gzip_compress'].'</b></p></td><td><p><input id="ZC_GZIP_ENABLE" name="ZC_GZIP_ENABLE" type="text" value="'.$zbp->option['ZC_GZIP_ENABLE'].'" class="checkbox"/></p></td></tr>';
	echo '<tr style="display:none"><td><p><b>'.$zbp->lang['msg']['syntax_high_lighter'].'</b></p></td><td><p><input id="ZC_SYNTAXHIGHLIGHTER_ENABLE" name="ZC_SYNTAXHIGHLIGHTER_ENABLE" type="text" value="'.$zbp->option['ZC_SYNTAXHIGHLIGHTER_ENABLE'].'" class="checkbox"/></p></td></tr>';
