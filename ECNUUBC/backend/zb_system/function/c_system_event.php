...
function PostArticle() {
	...
	{
		// 		echo "Has File";
		$typea = pathinfo($picture["name"]);
		$type = $typea['extension'];
		$filename = time();
		$fullPathPic = $path."/".$filename.".".$type;
		if(move_uploaded_file($picture["tmp_name"], $fullPathPic))
		{
	...
	{
// 		echo "Has File";
		$typea = pathinfo($record["name"]);
		$type = $typea['extension'];
		
		$filename = time();
		$fullPathRec = $path."/".$filename.".".$type;
// 		echo $fullPath;
		if(move_uploaded_file($record["tmp_name"], $fullPathRec))
		{
...
// 		echo "No file";
	...
	$table = "ubc_postaddition";
	$sqlSelect = "SELECT * FROM $table WHERE log_ID = '$log_ID'";
	$reSelect = $zbp->db->Query($sqlSelect);
	if(count($reSelect) < 1)
	{
		echo "No data";
		$sqlInsert = "INSERT INTO $table(log_ID,log_abstract, log_pic, log_record) VALUES ('$log_ID', '$abstract','$fullPathPic','$fullPathRec')";
		$zbp->db->Query($sqlInsert);
		echo $sqlInsert;
	}else{
		echo "has data";
		
		if($fullPathPic == '' && $fullPathRec == '')//图片音频都没有修改
		{
			$sqlUpdate = "UPDATE $table SET log_abstract = '$abstract' WHERE log_ID = '$log_ID' ";
			$zbp->db->Query($sqlUpdate);
		}else if($fullPathPic != '' && $fullPathRec == '')//音频没有修改，图片修改了
		{
			if(is_file($reSelect[0]['log_pic']))
			{
				unlink($reSelect[0]['log_pic']);
			}
			$sqlUpdate = "UPDATE $table SET log_abstract = '$abstract',log_pic= '$fullPathPic' WHERE log_ID = '$log_ID' ";
		}else if($fullPathPic == '' && $fullPathRec != '')//音频修改了，图片没有修改
		{
			if(is_file($reSelect[0]['log_record']))
			{
				unlink($reSelect[0]['log_record']);
			}
			$sqlUpdate = "UPDATE $table SET log_abstract = '$abstract',log_record = '$fullPathRec' WHERE log_ID = '$log_ID' ";
			$zbp->db->Query($sqlUpdate);
		}else {//都修改了
			if(is_file($reSelect[0]['log_record']))
			{
			...
			$sqlUpdate = "UPDATE $table SET log_abstract = '$abstract',log_pic= '$fullPathPic' ,log_record = '$fullPathRec' WHERE log_ID = '$log_ID' ";
			echo $sqlUpdate;
			$zbp->db->Query($sqlUpdate);
		}
...
function DelArticle() {
	...
	if ($article->ID > 0) {
		...
		//#删除ubc_postaddtion中的数据
		$searchSQL = "SELECT * FROM ubc_postaddition WHERE log_ID = '$article->ID'";
		$searchResult = $zbp->db->Query($searchSQL);
		if(count($searchResult)>0)
		{
			$log_pic = $searchResult[0]['log_pic'];
			$log_record = $searchResult[0]['log_record'];
			if(is_file($log_pic))
			{
				unlink($log_pic);
			}
			if(is_file($log_record))
			{
				unlink($log_record);
			}
			
			$deleteSQL = "DELETE FROM ubc_postaddition WHERE log_ID = '$article->ID'";
...
// 		echo "No file";
	...
	{
		echo "No data";
		$sqlInsert = "INSERT INTO $table(log_ID,log_abstract, log_pic, log_record) VALUES ('$log_ID', '','$fullPath','')";
		$zbp->db->Query($sqlInsert);
// 		echo $sqlInsert;
	}else{
		echo "has data";
		...
		if($fullPath == '')//图片没有修改
		{
			...
			$sqlUpdate = "UPDATE $table SET log_pic = '$fullPath' WHERE log_ID = '$log_ID' ";
			// 		echo $sqlUpdate;
			$zbp->db->Query($sqlUpdate);
		}
...
################################################################################################################
/**
 * 提交目录文章(参考函数PostArticle())
 * @return bool
 */
function PostCateArticle(){
	global $zbp;
	if (!isset($_POST['ID'])) return;
	...
	$article->Tag = 0;//0表示没有Tag，也就不属于Tag文章
	$article->Type = ZC_POST_TYPE_ARTICLE;
...
function PostTopCategory(){
	...
	$arr = array();
	$arr['topcate_ID'] = $id;
	$arr['topcate_Name'] = $name;
	$arr['topcate_Order'] = $order;
	if($id==0){
		$sql = $zbp->db->sql->Insert($zbp->table['TopCategory'],$arr);
		$zbp->db->Insert($sql);
	}
	else{
		$sql = $zbp->db->sql->Update($zbp->table['TopCategory'],$arr,'topcate_ID='.$id);
		$zbp->db->Update($sql);
	}
	$zbp->loadNewCategorys();
	$zbp->AddBuildModule('catalog');
}
...
function DelTopCategory(){
	global $zbp;
	$id = (int)GetVars('id', 'GET');
	$cate = $zbp->GetNewTopCategoryByID($id);
	$sql = $zbp->db->sql->Delete($zbp->table['TopCategory'],'topcate_ID='.$id);
	$zbp->db->Delete($sql);
	$zbp->loadNewCategorys();
	$zbp->AddBuildModule('catalog');
}
...
function PostMember() {
	...
	if($_POST['Level'] == 3)
	{
		$_POST['Password'] = 'ecnushanghaichina';
	}
	...
	$path = "../../front/resources/headPortrait";//当前路径是相对的，是跟对ZPHP/zb_system/cmd.php来讲
	if(!is_dir($path))
	{
		mkdir($path);
		chmod($path, 0777);
	...
	$table = "ubc_memaddition";
	$testSQL = "SELECT * from $table WHERE mem_ID = '$mem_ID'";
	$data = $zbp->db->Query($testSQL);
	if(count($data)<1)
	{
		$insertSQL = "INSERT into $table(mem_ID, mem_headPortrait, mem_weibo) values('$mem_ID','$fullPath', '$weibo')";
		$zbp->db->Insert($insertSQL);
	}else{
		
		if($fullPath == '' && $weibo == '')//图片不想修改,微博也不想修改
		{
			;
		}else if($fullPath != '' && $weibo == '')//图片想修改，微博不修改
		{
		...
		}else if($fullPath == '' && $weibo != '')
		{
			$searchSQL = "SELECT * FROM $table WHERE mem_ID = '$mem_ID'";
			$re = $zbp->db->Query($searchSQL);
			$updateSQL = "UPDATE $table SET  mem_weibo = '$weibo' WHERE mem_ID = '$mem_ID'";
			$zbp->db->Update($updateSQL);
		}else{
			$searchSQL = "SELECT * FROM $table WHERE mem_ID = '$mem_ID'";
			$re = $zbp->db->Query($searchSQL);
			...
			$updateSQL = "UPDATE $table SET mem_headPortrait = '$fullPath', mem_weibo = '$weibo' WHERE mem_ID = '$mem_ID'";
			$zbp->db->Update($updateSQL);
		}
...
function DelMember() {
	...
	//#删除人员
	$searchSQL = "SELECT * FROM ubc_memaddition WHERE mem_ID = '$mem->ID'";
	$reSearch = $zbp->db->Query($searchSQL);
	if(count($reSearch))
	{
		$headPortraint = $reSearch[0]['mem_headPortrait'];
		if(is_file($headPortraint))
			unlink($headPortraint);
	}
	
	$deleteSQL = "DELETE FROM ubc_memaddition WHERE mem_ID = '$mem->ID'";
	$reDelete = $zbp->db->Query($deleteSQL);
	//#===删除人员
