...
class ZBlogPHP {
	...
	//public $topCategorys = array();
	/**
	 * @var array 分类数组（已排序）
	 */
	public $categorysbyorder=array();
	
	//public $topCategorysbyorder = array();
	/**
	 * @var int 分类最大层数
	 */
	...
	public $newCategorys = array();
	
	public $newTopCategorys = array();
...
#初始化
	...
	public function loadNewCategorys(){
		//导入二级分类到 $newCategorys
		$sql = $this->db->sql->Select($this->table['Category'].",".$this->table['TopCategory'],'*','topcate_ID=cate_ParentID',array('cate_Order'=>'ASC'),null,null);
		
		$this->newCategorys = $this->db->Query($sql);
		
		//导入一级分类到$newTopCategorys
		$sql = $this->db->sql->Select($this->table['TopCategory'],'*',null,null,null,null);
		$newLocalNewTopCategorys = $this->db->Query($sql);
		foreach($newLocalNewTopCategorys as $k=>$v){
			$this->newTopCategorys[$v['topcate_ID']] = $v;
		}
	}
...
#加载函数
	...
	public function LoadTags(){
		...
		$sql = $this->db->sql->Select($this->table['Tag'],null,null,null,null,null);
		$arr = $this->db->Query($sql);
		foreach ($arr as $t) {
			$this->newTags[$t['tag_ID']]=$t;
		}
	}
...
#加载数据对像List函数
	...
	function GetArticleList($select=null,$where=null,$order=null,$limit=null,$option=null,$readtags=true){
		...
		if(is_array($where))array_unshift($where,array('=','log_CateID','0'));
		$sql = $this->db->sql->Select($this->table['Post'],$select,$where,$order,$limit,$option);
	 ...
	 * 获得目录文章列表
	 */
	function GetCateArticleList($select=null,$where=null,$order=null,$limit=null,$option=null,$readtags=true){
	
		if(empty($select)){$select = array('*');}
		if(empty($where)){$where = array();}
		if(is_array($where))array_unshift($where,array('=','log_Type','0'));
		if(is_array($where))array_unshift($where,array('=','log_Tag','0'));
		$sql = $this->db->sql->Select($this->table['Post'],$select,$where,$order,$limit,$option);
		// 		echo $sql;
		$array = $this->GetList('Post',$sql);
	...
	function GetCategoryList($select=null,$where=null,$order=null,$limit=null,$option=null){
		if(empty($select)){$select = array('*');}
		$sql = $this->db->sql->Select($this->table['Category'].",".$this->table['TopCategory'],$select,$where,$order,$limit,$option);
		
		//$sql = "SELECT * FROM $c,$tc WHERE $c.cate_ParentID=$tc.topcate_ID";
...
#读取对象函数
	...
	function GetNewTopCategoryByID($id){
		if(isset($this->newTopCategorys[$id])){
			return $this->newTopCategorys[$id];
		}else{
			return array();
		}
	}
