...
#引入必备
$basepath = dirname(__FILE__) . DIRECTORY_SEPARATOR;//F定义当前文件所在服务器的绝对路径，应该是ZBP/zb_system/function这个目录
define('ZBP_PATH',str_replace('\\','/',realpath($basepath . '..' . DIRECTORY_SEPARATOR . '..' . DIRECTORY_SEPARATOR)) . '/');//F找到现在的www的路径，对于xampp就是D:/xampp/htdocs/

require $basepath . 'c_system_plugin.php';//F定义了相关的插件，并且将插件信息保存在文件中的全局变量中
require $basepath . 'c_system_debug.php';//F定义了相关的调试信息
require $basepath . 'c_system_common.php';//F定义了一些可能会使用的函数
require $basepath . 'c_system_event.php';//F定义了一些相关事件的反应
...
$option = require($blogpath . 'zb_system/defend/option.php');//F系统配置的参数，保存在option这个数组中。
foreach ($option_zbusers as $key => $value) {//F用户配置保存在系统配置的数组中
	$option[$key] = $value;
}
unset($basepath,$key,$value,$option_zbusers);//F unset，销毁指定的变量。在这里是销毁多个变量
...
#加载zbp 数据库类 基础对象
AutoloadClass('ZBlogPHP');//F ZBlogPHP是全局操作类
AutoloadClass('DbSql');//F 加载数据库操作类
AutoloadClass('Base');//F 加载数据操作基类


#实例化zbp
$zbp=ZBlogPHP::GetInstance();
$zbp->Initialize();//F 完成对系统的初始化
...
$ap=array_unique($ap);//F 删除中数组中存在相同值的元素
